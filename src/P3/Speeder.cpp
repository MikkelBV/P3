...
double Speeder::process(int method, int framesToSkip, bool resizeVideo) {
	switch (method) {
		...
		case METHOD_FEATUREMATCHING:
			return methodFeatureMatching(framesToSkip, resizeVideo);
...
double Speeder::methodFeatureMatching(int framesToSkip, bool resizeVideo) {
	sequence->restart();
	Mat frame = sequence->nextFrame();
...
double Speeder::methodKalmanFeatures(int framesToSkip, bool resizeVideo) {
	sequence->restart();
	Mat frame = sequence->nextFrame();
